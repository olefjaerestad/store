...
describe('Store', () => {
	...
	it('Should be subscribeable.', () => {
		...
		const subscriber = (prop: keyof typeof store.state) => prop === 'name' ? isSubscribed = true : null;
	...
	it('Specific props should be subscribeable.', () => {
		let isSubscribed = false;
		const subscriber = (prop: keyof typeof store.state) => isSubscribed = !isSubscribed;
	...
	it('Subscribed callbacks registered without specific properties should not run on nested property changes.', () => {
		let isSubscribed = false;
		const subscriber = (prop: string | number | symbol) => prop === 'foo' ? isSubscribed = true : null;
		store.subscribe(subscriber);
